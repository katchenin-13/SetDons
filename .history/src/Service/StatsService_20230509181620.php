...
use Doctrine\Persistence\ObjectManager;
 ...
 class StatsService {
     ...
     * @return void
     */
    public function getAudience($debut ,$fin,$comm=null)
    {
    // $query = $this->manager->createQuery('SELECT u FROM ForumUser u WHERE u.username = :name');
    // return $this->manager->createQueryBuilder('SELECT COUNt(d)  FROM App\Entity\Don AS d,App\Entity\Beneficiaire AS b,App\Entity\Communaute AS c
    //           WHERE d.id = b.don
    //           AND b.communaute = c.id AND d.CreatedAt BETWEEN :debut AND :fin
    //          GROUP BY c.id')
   ...
   ;
    return $this->manager->createQueryBuilder('SELECT COUNt(a)  FROM App\Entity\Audience AS a,App\Entity\Communaute AS c
              WHERE a.id = a.communaute
              AND bAND d.CreatedAt BETWEEN :debut AND :fin
             GROUP BY c.id')
            ->setParameter(':debut', $debut)
            ->setParameter(':fin', $fin)
            ->getQuery()->getSingleScalarResult();
