...
use function PHPUnit\Framework\classHasAttribute;
 ...
 class StatsService {
    ...
    public function getAudienceMoisCom($id)
    {
        ...
        $repo = $this->manager->getRepository(Audience::class)->createQueryBuilder('a');
        return $repo->select('count(a.id) as nbre', "DATE_FORMAT(a.CreatedAt,'%M') as mois","c.libelle")
    ...
    public function getAudienceAnlocalie($id)
    {
        ...
        $repo = $this->manager->getRepository(Audience::class)->createQueryBuilder('a');
        return $repo->select('count(a.id) as nbre', "DATE_FORMAT(a.CreatedAt,'%Y') as annee","l.libelle")
        ->innerJoin('a.communaute', 'c')
    ...
    public function getAudienceMoisLocalie($id)
    {
        ...
        $repo = $this->manager->getRepository(Audience::class)->createQueryBuilder('a');
        return $repo->select('d', "DATE_FORMAT(a.CreatedAt,'%M') as mois", "c.libelle")
        ->innerJoin('a.communaute', 'c')
        ->innerJoin('c.localite', 'l')
        ->groupBy('mois')
        ->andWhere('l.id=:id')
