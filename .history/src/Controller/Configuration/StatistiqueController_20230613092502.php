...
class StatistiqueController extends AbstractController
{
    ...
    public function dataTypeContrat(Request $request,StatsService $statsService, CommunauteRepository $com)
    {
        $all = $request->query->all();
        dd($all);
        $filters = $all['filters'] ?? [];
        $typeContratId = $filters['communaute'];
        $dataAnnees =   $statsService->getAudienceMoisCom($typeContratId);

        dd($dataAnnees);
        $annees = range($dataAnnees['min_year'], $dataAnnees['max_year']);
        $data = $employeRepository->getDataTypeContrat($typeContratId);

        $typeContrat = $->find($typeContratId);

        $series = [['name' => $communaute->getLibelle(), 'data' => []]];
