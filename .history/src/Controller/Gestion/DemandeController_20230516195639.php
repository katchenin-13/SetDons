...
class DemandeController extends AbstractController
{
    ...
    public function index(Request $request, DataTableFactory $dataTableFactory): Response
    {
        ...
        $renders = [
            'statusdemande_bloquer' =>  new ActionRender(function () {
                return true;
            }),
            'statusdemande_debloquer' =>  new ActionRender(function () {
                return true;
            }),
            ...
            $table->add('id', TextColumn::class, [
                ...
                , 'render' => function ($value, Demande $context) use ($renders) {
                    $options = [
                        ...
                        'actions' => [
                            'statusdemande_bloquer' => [
                                'url' => $this->generateUrl('acte_demande', ['id' => $value]), 'ajax' => false, 'icon' => '%icon% fa  fa-unlock-alt', 'attrs' => ['class' => 'btn-main test'], 'render' => new ActionRender(function () use ($context) {
                                    return $context->isStatusdemande();
                                })
                            ],
                            'statusdemande_debloquer' => [
                                'url' => $this->generateUrl('acte_demande', ['id' => $value]), 'ajax' => false, 'icon' => '%icon% fa fa-lock', 'attrs' => ['class' => 'btn-danger active test'], 'render' => new ActionRender(function () use ($context) {
                                    return !$context->isStatusdemande();
                                })
                            ],
                            'show' => [
                                'url' => $this->generateUrl('app_gestion_deman_show', ['id' => $value]), 'ajax' => true, 'icon' => '%icon% bi bi-eye', 'attrs' => ['class' => 'btn-success'], 'render' => $renders['edit']
                            ],
