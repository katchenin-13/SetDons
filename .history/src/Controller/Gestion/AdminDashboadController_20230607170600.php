...
class AdminDashboadController extends AbstractController
 {
    ...
    public function index(EntityManagerInterface $mamager,StatsService $statsService,Request $request) :Response
    {
        ...
        $audiences = $statsService->getAudience('2023-04-09', '2023-05-10');
         $audienceyes = $st->getAudiencerealieyes();
        $audienceno =$st->getAudiencerealieno();
        $audiencemois =$st->getAudienceMois();
        $audienceannee = $st->getAudienceAnnee();
        $audiencesmoiscom = $st->getAudienceMoisCom();
        $audiencesanncom = $st ->getAudienceAnCom();
        $audiencesmoisloca = $st->getAudienceMoisLocalie();
        $audienceanneloca =$st->getAudienceAnlocalie();
        $$audiencemoiscateg =$st->getAudienceMoisCetagorie();
        // dd($audiences);
        // $audiences = $annRepo->countByDate();
