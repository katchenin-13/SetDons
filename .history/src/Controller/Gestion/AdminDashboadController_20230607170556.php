...
class AdminDashboadController extends AbstractController
 {
    ...
    public function index(EntityManagerInterface $mamager,StatsService $statsService,Request $request) :Response
    {
        ...
        $audiences = $statsService->getAudience('2023-04-09', '2023-05-10');
         $audienceyes = $->getAudiencerealieyes();
        $audienceno =$->getAudiencerealieno();
        $audiencemois =$->getAudienceMois();
        $audienceannee = $->getAudienceAnnee();
        $audiencesmoiscom = $->getAudienceMoisCom();
        $audiencesanncom = $ ->getAudienceAnCom();
        $audiencesmoisloca = $->getAudienceMoisLocalie();
        $audienceanneloca =$->getAudienceAnlocalie();
        $$audiencemoiscateg =$->getAudienceMoisCetagorie();
        // dd($audiences);
        // $audiences = $annRepo->countByDate();
