...
class AdminDashboadController extends AbstractController
 {
    ...
    #[Route('/api/{id}/chois', name: 'api_dashboad')]
...
4        ], 200);
    ...
    #[Route('/', name: 'app_gestion_admin_dashboad')]
    public function index(Request $request) :Response
    {
      
      // dd($this->id);
   
      if (   $this->id = $request->get('id')) {
            dd($this->statsService->getAudienceMois());
      }           
        ...
        foreach ($this->statsService->getAudiencerealieyes() as $cam) {
            $mois[] = $cam['mois'];
            $y[] = $cam['nbre'];
        }
