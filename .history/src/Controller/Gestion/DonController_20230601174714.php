...
class DonController extends AbstractController
{
    ...
    public function index(Request $request, DataTableFactory $dataTableFactory,StatsService $fieldonRepository,Fieldon $fieldon): Response
    {  
        $id = $fieldon->getTypedon();
        $i = count($id)
    
        for ($i=0; $i <= $id ; $i++) {
            $field = $fieldonRepository->$VarName = $repo->findOneBy(['property' => 'value']);(array('typedon' => $i));
