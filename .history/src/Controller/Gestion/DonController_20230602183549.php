...
class DonController extends AbstractController
{
    ...
    //  * @return Response
    //  */
    // public function active(Don $parent, EntityManagerInterface $entityManager): Response
    // {
    ...
    // /**
    //  * cette fonction permet de generer un pdf de don
    //  *
    //  * @param DonRepository $don
    //  * @return Response
    //  */
    // #[Route('/pdf/generator', name: 'app_pdf_generator')]
    // public function generatePdf(DonRepository $don): Response
    // {
    //     $data = $don->findAll();
    //     $html =  $this->renderView('gestion/don/detail.html.twig', [
    //         'data' => $data
    //     ]);
    //     $dompdf = new Dompdf();
    ...
    public function index(Request $request,FieldonRepository $fieldonRepository, DataTableFactory $dataTableFactory,StatsService $statsService): Response
    {  

        //$fildon = $statsService->getfieldon(7,1);
        dd($fieldonRepository->listFieldByGroup(7,1));
                ...
                'label' => 'Actions', 'orderable' => false, 'globalSearchable' => false, 'className' => 'grid_row_actions', 'render' => function ($value, Don $context) use ($renders) {
                    $options = [
                        ...
                        'actions' => [
                            ...
                            // ],
                            'show' => [
                                'url' => $this->generateUrl('app_gestion_don_show', ['id' => $value]), 'ajax' => true, 'icon' => '%icon% bi bi-eye', 'attrs' => ['class' => 'btn-success'], 'render' => $renders['edit']
                            ],
                            'edit' => [
    ...
    public function show(Don $don,StatsService $statsService): Response
    {    
        return $this->render('gestion/don/show.html.twig', [
            'don' => $don,
            'espece' => $statsService->getfieldon($don, 1),
            'nature' => $statsService->getfieldon($don,2),
        ]);
    }
